...
import StickyBarItem from "../StickyBarItem/StickyBarItem";
            ...
            <Image
              loader={() =>
                `${process.env.NEXT_PUBLIC_API}media/${
                  cv.cv.photo && cv.cv.photo
                }`
              }
              src={`${process.env.NEXT_PUBLIC_API}media/${
                cv.cv.photo && cv.cv.photo
              }`}
              alt={cv.cv.alt}
              id={cv.cv.id}
          ...
          {cv.cv && (
            <>
              <StickyBarItem
                src={email}
                title="email"
                content={`${cv.cv.email && cv.cv.email}`}
              />
              <StickyBarItem
                src={phone}
                title="phone"
                content={`${cv.cv.photo && cv.cv.phone}`}
              />
            </>
          )}
          <StickyBarItem
            src={calendar}
            title="birthday"
            content="April 5th, 1998"
          />
        </div>
        <div
          className="stickyBar-button"
          onClick={() => document.getElementById("downloadCV")?.click}
        >
          {cv.cv && (
            <Link href={`${process.env.NEXT_PUBLIC_API}media/${cv.cv.CV}`}>
              <a id={"downloadCV"} download>
                <button className={`button`}>
                  <span className={`button-span`}>Download CV</span>
                </button>
              </a>
            </Link>
          )}
